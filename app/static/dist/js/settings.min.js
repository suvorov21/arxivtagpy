...
$("#add-cat-btn").click(()=>{let cat=document.forms["add-cat"]["cat_name"].value;if(CATS.includes(cat)){raiseAlert("Catagory already added!","danger");return;}
if(typeof(allCatsArray[cat])==="undefined"){raiseAlert("Unknown category","danger");return;}
...
$(event.target).css("border-color","#000");if($(event.target).attr("id")==="add-tag"){newTag=true;$("#btn-reset").click();$("#add-tag").css("border-style","solid");$("#add-tag").css("border-width","4px");document.forms["add-tag"]["tag_name"].value="";document.forms["add-tag"]["tag_rule"].value="";document.forms["add-tag"]["tag_color"].value="";$("#btn-del").addClass("disabled");}else{newTag=false;let editTagName=$(event.target).attr("id").split("-").slice(2);editTagName=editTagName.join("-");editTagId=findTagIdByName(editTagName);let tag=TAGS[parseInt(editTagId,10)];$("#tag-fields").prop("disabled",false);document.forms["add-tag"]["tag_name"].value=tag.name;document.forms["add-tag"]["tag_rule"].value=tag.rule;document.forms["add-tag"]["tag_color"].value=tag.color;$("#tag-color").css("background-color",$("#tag-color").val());$("#btn-del").removeClass("disabled");}
...
let url="mod_pref";let dataSet={"tex":document.getElementById("tex-check").checked,"dark":document.getElementById("radio-dark").checked};$.post(url,JSON.stringify(dataSet)).done(function(){reloadSettings();$(".btn-save").addClass("disabled");raiseAlert("Settings are saved","success");var links=document.getElementsByTagName("link");for(var i=0;i<links.length;i++){var link=links[parseInt(i,10)];if(link.rel==="stylesheet"){link.href+="?";}}
window.location.reload(true);return false;}).fail(function(){raiseAlert("Settings are not saved. Please try later","danger");return false;});return false;}
