...
if(confirm("Are you sure you want to delete "+TAGS[parseInt(editTagId,10)].name+"?")){TAGS.splice(editTagId,1);submitTag();event.preventDefault();}else{event.preventDefault();return;}});$(".form-check-input").change(()=>{$(".btn-save").removeClass("disabled");});function fillSetForm(){let url="mod_pref";let dataSet={"tex":document.getElementById("tex-check").checked
};$.post(url,JSON.stringify(dataSet)).done(function(data){reloadSettings();$(".btn-save").addClass("disabled");raiseAlert("Settings are saved","success");return false;}).fail(function(jqXHR){raiseAlert("Settings are not saved. Please try later","danger");return false;});return false;}
