...
$("#tag-fields").prop("disabled",false);});$(".tag-field").on("input",function(){$(".btn-save").removeClass("disabled");});$("#tag-color").on("change",function(){$("#tag-color").css("background-color",$("#tag-color").val());$(".btn-save").removeClass("disabled");});function submitTag(){let url="mod_tag";$.post(url,JSON.stringify(TAGS)).done(function(data){reloadSettings();$(".btn-save").addClass("disabled");$("#btn-del").addClass("disabled");return true;}).fail(function(jqXHR){alert(jqXHR);return false;});}
