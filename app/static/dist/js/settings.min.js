...
prefs.save();function raiseAlert(text="Text",type="alert"){let parent=document.createElement("div");parent.setAttribute("class","alert alert-dismissible fade show alert-"+type);parent.setAttribute("role","alert");let content=document.createElement("span");content.textContent=text;let close=document.createElement("button")
close.setAttribute("class","close");close.setAttribute("data-dismiss","alert");close.setAttribute("aria-label","Close");let time=document.createElement("span");time.innerHTML="&times;";document.body.insertBefore(parent,document.getElementById("main-container"));parent.appendChild(content);parent.appendChild(close);close.appendChild(time);}
...
return;}
function reloadSettings(){if($("#cats-link").hasClass("active")){if(CATS!==catNew){renderCats();}}
...
window.onload=function(){$.each(allCatsArray,function(val,text){$("#catsDataList").append($("<option>").attr("value",val).text(text));});reloadSettings();}
...
$(".btn-save").removeClass("disabled");addCat(document.forms["add-cat"]["cat_name"].value);document.forms["add-cat"]["cat_name"].value="";catNew.push(cat);});$(".btn-cancel").click(()=>{if(!$(".btn-cancel").hasClass("disabled")){location.reload();}});$("#show-rules").click(()=>{if($("#tag-help").css("display")==="block"){$("#tag-help").css("display","none");}else{$("#tag-help").css("display","block");}});var newTag=true;var editTagId=-1;$("#tag-list").click((event)=>{if(typeof($(event.target).attr("class"))==="undefined"){return;}
if(!$(event.target).attr("class").includes("tag-label")){return;}
...
$("#tag-fields").prop("disabled",false);});$(".tag-field").on("input",function(){$(".btn-save").removeClass("disabled");});$("#tag-color").on("change",function(){$("#tag-color").css("background-color",$("#tag-color").val());$(".btn-save").removeClass("disabled");});function submitTag(){let url="mod_tag";$.post(url,JSON.stringify(TAGS)).done(function(data){reloadSettings();$(".btn-save").addClass("disabled");$("#btn-del").addClass("disabled");raiseAlert("Settings are saved","success");return true;}).fail(function(jqXHR){raiseAlert("Settings are not saved. Please try later","danger");return false;});}
...
$("#btn-del").click((event)=>{if(newTag){event.preventDefault();return;}
if(confirm("Are you sure you want to delete "+TAGS[parseInt(editTagId,10)].name+"?")){TAGS.splice(editTagId,1);submitTag();event.preventDefault();}else{event.preventDefault();return;}});$(".form-check-input").change((event)=>{$(".btn-save").removeClass("disabled");});function fillSetForm(){let url="mod_pref";let data_set={"tex":document.getElementById("tex-check").checked,"easy_and":document.getElementById("and-check").checked}
$.post(url,JSON.stringify(data_set)).done(function(data){reloadSettings();$(".btn-save").addClass("disabled");raiseAlert("Settings are saved","success");return false;}).fail(function(jqXHR){raiseAlert("Settings are not saved. Please try later","danger");return false;});return false;}
