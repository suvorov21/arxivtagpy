...
prefs.save();function raiseAlert(text="Text",type="alert"){let parent=document.createElement("div");parent.setAttribute("class","alert alert-dismissible fade show alert-"+type);parent.setAttribute("role","alert");let content=document.createElement("span");content.textContent=text;let close=document.createElement("button");close.setAttribute("class","close");close.setAttribute("data-dismiss","alert");close.setAttribute("aria-label","Close");let time=document.createElement("span");time.innerHTML="&times;";document.body.insertBefore(parent,document.getElementById("main-container"));parent.appendChild(content);parent.appendChild(close);close.appendChild(time);}
...
parent.ondragover=function(event){let target=event.target.getAttribute("id").split("-").slice(2);dragTarget=target.join("-").replaceAll("111",".");}
let close=document.createElement("button");close.setAttribute("id","close_"+cat.replaceAll(".","111"));close.setAttribute("class","close close-btn");close.innerHTML="&times";close.onclick=function(event){let name=event.target.getAttribute("id").split("_")[1];$("#par-cat-"+name).removeClass("d-flex");$("#par-cat-"+name).fadeOut();$(".btn").removeClass("disabled");const catId=CATS.indexOf(name.replace("111","."));if(catId>-1){CATS.splice(catId,1);}};let catElement=document.createElement("div");catElement.setAttribute("class","pl-2");catElement.setAttribute("id","cat-name-"+cat.replaceAll(".","111"));catElement.textContent=allCatsArray[cat];document.getElementById("cats-list").appendChild(parent);parent.appendChild(close);parent.appendChild(catElement);}
function renderCats(){$("#cats-list").empty();CATS.forEach((cat)=>{addCat(cat);});document.getElementById("cats-list").ondragover=function(event){event.preventDefault();}
document.getElementById("cats-list").ondrop=function(event){event.preventDefault();let moved=event.dataTransfer.getData("Text");let movedId=CATS.indexOf(moved);let targetId=CATS.indexOf(dragTarget);CATS[parseInt(movedId,10)]=dragTarget;CATS[parseInt(targetId,10)]=moved;$(".btn").removeClass("disabled");reloadSettings();}}
...
let moved=event.target.getAttribute("id").split("-").slice(2);moved=moved.join("-").replaceAll("111","-");moved=findTagIdByName(moved);event.dataTransfer.setData("Text",moved);}
tagElement.ondragover=function(event){let target=event.target.getAttribute("id").split("-").slice(2);dragTarget=target.join("-").replaceAll("111","-");dragTarget=findTagIdByName(dragTarget);}});document.getElementById("tag-list").ondragover=function(event){event.preventDefault();}
document.getElementById("tag-list").ondrop=function(event){event.preventDefault();let moved=event.dataTransfer.getData("Text");let movedId=parseInt(moved,10);let targetId=parseInt(dragTarget,10);let buffer=TAGS[movedId];TAGS[parseInt(movedId,10)]=TAGS[targetId];TAGS[parseInt(targetId,10)]=buffer;$(".btn").removeClass("disabled");reloadSettings();}
...
if(PREF["easy_and"]){document.getElementById("and-check").checked=true;}
return;}
...
window.onload=function(){$.each(allCatsArray,function(val,text){$("#catsDataList").append($("<option>").attr("value",val).text(text));});reloadSettings();};function submitCat(){let url="mod_cat"
$.post(url,{"list":CATS}).done(function(){CATS=Array.from(CATS);reloadSettings();$(".btn-save").addClass("disabled");raiseAlert("Settings are saved","success");return false;}).fail(function(){raiseAlert("Settings are not saved. Please try later","danger");return false;});return false;}
...
$(".btn-save").removeClass("disabled");addCat(document.forms["add-cat"]["cat_name"].value);document.forms["add-cat"]["cat_name"].value="";CATS.push(cat);});$(".btn-cancel").click(()=>{if(!$(".btn-cancel").hasClass("disabled")){location.reload();}});$("#show-rules").click(()=>{if($("#tag-help").css("display")==="block"){$("#tag-help").css("display","none");}else{$("#tag-help").css("display","block");}});var newTag=true;var editTagId=-1;const findTagIdByName=(name)=>{for(let tagId=0;tagId<TAGS.length;tagId++){if(TAGS[tagId]["name"]===name){return tagId;}}
return-1;}
$("#tag-list").click((event)=>{if(typeof($(event.target).attr("class"))==="undefined"||!$(event.target).attr("class").includes("tag-label")){return;}
...
$("#tag-fields").prop("disabled",false);});function makeTagEdited(){$(".btn-save").removeClass("disabled");tagEdited=true;var doms=document.getElementsByClassName("tag-label");for(i=0;i<doms.length;i++){doms[i].style.cursor="not-allowed";}}
...
function checkTag(){$(".cat-alert").empty();if(document.forms["add-tag"]["tag_name"].value===""||document.forms["add-tag"]["tag_rule"].value===""||document.forms["add-tag"]["tag_color"].value===""){$(".cat-alert").html("Fill all the fields in the form!");return false;}
...
if(confirm("Are you sure you want to delete "+TAGS[parseInt(editTagId,10)].name+"?")){TAGS.splice(editTagId,1);submitTag();event.preventDefault();}else{event.preventDefault();return;}});$(".form-check-input").change(()=>{$(".btn-save").removeClass("disabled");});function fillSetForm(){let url="mod_pref";let dataSet={"tex":document.getElementById("tex-check").checked,"easy_and":document.getElementById("and-check").checked};$.post(url,JSON.stringify(dataSet)).done(function(data){reloadSettings();$(".btn-save").addClass("disabled");raiseAlert("Settings are saved","success");return false;}).fail(function(jqXHR){raiseAlert("Settings are not saved. Please try later","danger");return false;});return false;}
