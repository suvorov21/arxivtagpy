...
let url="mod_pref";let dataSet={"tex":document.getElementById("tex-check").checked,"dark":document.getElementById("radio-dark").checked};$.post(url,JSON.stringify(dataSet)).done(function(data){reloadSettings();$(".btn-save").addClass("disabled");raiseAlert("Settings are saved","success");var links=document.getElementsByTagName("link");for(var i=0;i<links.length;i++){var link=links[i];if(link.rel==="stylesheet"){link.href+="?";}}
window.location.reload()
return false;}).fail(function(jqXHR){raiseAlert("Settings are not saved. Please try later","danger");return false;});return false;}
