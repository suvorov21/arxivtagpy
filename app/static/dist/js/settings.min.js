...
$(".btn-save").removeClass("disabled");addCat(document.forms["add-cat"]["cat_name"].value);document.forms["add-cat"]["cat_name"].value="";catNew.push(cat);});$(".btn-cancel").click((event)=>{location.reload();});$(".nav-link").click((event)=>{if(!$(".btn-cancel").hasClass("disabled")){if(confirm("Settings will not be saved. Continue?")){$(".btn-save").addClass("disabled");reloadSettings();event.preventDefault();}else{event.preventDefault();return;}}
$("#cats-set").css("display","none");$("#tags-set").css("display","none");$("#pref-set").css("display","none");$("#cats-link").removeClass("active");$("#tags-link").removeClass("active");$("#pref-link").removeClass("active");$("#"+event.target.getAttribute("id").split("-")[0]+"-set").css("display","block");$(event.target).addClass("active");});