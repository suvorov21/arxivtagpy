...
document.getElementById("cats-list").ondrop=function(event){event.preventDefault();let moved=event.dataTransfer.getData("Text");let movedId=CATS.indexOf(moved);let targetId=CATS.indexOf(dragTarget)
CATS[movedId]=dragTarget;CATS[targetId]=moved;$(".btn").removeClass("disabled");reloadSettings();}}
...
document.getElementById("tag-list").ondrop=function(event){event.preventDefault();let moved=event.dataTransfer.getData("Text");let movedId=parseInt(moved,10);let targetId=parseInt(dragTarget,10);let buffer=TAGS[movedId];TAGS[movedId]=TAGS[targetId];TAGS[targetId]=buffer;$(".btn").removeClass("disabled");reloadSettings();}
...
let TagDict={"name":document.forms["add-tag"]["tag_name"].value,"rule":document.forms["add-tag"]["tag_rule"].value,"color":document.forms["add-tag"]["tag_color"].value};if(!newTag){TAGS[editTagId]["name"]=TagDict["name"];TAGS[editTagId]["rule"]=TagDict["rule"];TAGS[editTagId]["color"]=TagDict["color"];}else{TAGS.push(TagDict);}
submitTag();}
