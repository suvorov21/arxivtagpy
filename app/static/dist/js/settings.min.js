...
var catNew=[];var dragTarget;function addCat(cat){let parent=document.createElement("div");parent.setAttribute("class","d-flex cat-parent");parent.setAttribute("id","par-cat-"+cat.replaceAll(".","111"));parent.draggable=true;parent.ondragstart=function(event){let moved=event.target.getAttribute("id").split("-");moved=moved.slice(2);moved=moved.join("-").replaceAll("111",".");event.dataTransfer.setData("Text",moved);}
parent.ondragover=function(event){let target=event.target.getAttribute("id").split("-");target=target.slice(2);dragTarget=target.join("-").replaceAll("111",".");}
let close=document.createElement("button");close.setAttribute("id","close_"+cat.replaceAll(".","111"));close.setAttribute("class","close close-btn");close.innerHTML="&times";close.onclick=function(event){let name=event.target.getAttribute("id").split("_")[1];$("#par-cat-"+name).removeClass("d-flex");$("#par-cat-"+name).fadeOut();$(".btn").removeClass("disabled");const catId=catNew.indexOf(name.replace("111","."));if(catId>-1){catNew.splice(catId,1);}};let catElement=document.createElement("div");catElement.setAttribute("class","pl-2");catElement.setAttribute("id","cat-name-"+cat.replaceAll(".","111"));catElement.textContent=allCatsArray[cat];document.getElementById("cats-list").appendChild(parent);parent.appendChild(close);parent.appendChild(catElement);}
document.getElementById("cats-list").ondragover=function(event){event.preventDefault();}
document.getElementById("cats-list").ondrop=function(event){event.preventDefault();let moved=event.dataTransfer.getData("Text");let movedId=catNew.indexOf(moved);let targetId=catNew.indexOf(dragTarget)
catNew[movedId]=dragTarget;catNew[targetId]=moved;$(".btn").removeClass("disabled");CATS=catNew;reloadSettings();}
function renderCats(){$("#cats-list").empty();catNew=[];CATS.forEach((cat)=>{addCat(cat);catNew.push(cat);});}
...
else if($("#tags-link").hasClass("active")){renderTags();document.forms["add-tag"]["tag_name"].value="";document.forms["add-tag"]["tag_rule"].value="";document.forms["add-tag"]["tag_color"].value="";document.forms["add-tag"]["tag_order"].value="";}
...
$.post(url,{"list":catNew}).done(function(){CATS=Array.from(catNew);reloadSettings();$(".btn-save").addClass("disabled");raiseAlert("Settings are saved","success");return false;}).fail(function(){raiseAlert("Settings are not saved. Please try later","danger");return false;});return false;}
function fillCatForm(){if($(".btn-cancel").hasClass("disabled")){return false;}
submitCat();return false;}
$("#add-cat-btn").click(()=>{let cat=document.forms["add-cat"]["cat_name"].value;if(catNew.includes(cat)){alert("Catagory already added!");return;}
if(typeof(allCatsArray[cat])==="undefined"){alert("Unknown category");return;}
$(".btn-save").removeClass("disabled");addCat(document.forms["add-cat"]["cat_name"].value);document.forms["add-cat"]["cat_name"].value="";catNew.push(cat);});$(".btn-cancel").click(()=>{if(!$(".btn-cancel").hasClass("disabled")){location.reload();}});$("#show-rules").click(()=>{if($("#tag-help").css("display")==="block"){$("#tag-help").css("display","none");}else{$("#tag-help").css("display","block");}});var newTag=true;var editTagId=-1;$("#tag-list").click((event)=>{if(typeof($(event.target).attr("class"))==="undefined"||!$(event.target).attr("class").includes("tag-label")){return;}
if(!$(".btn-cancel").hasClass("disabled")){if(confirm("Settings will not be saved. Continue?")){$(".btn-save").addClass("disabled");event.preventDefault();}else{event.preventDefault();return;}}
...
$("#tag-fields").prop("disabled",false);});$(".tag-field").on("input",function(){$(".btn-save").removeClass("disabled");});$("#tag-color").on("change",function(){$("#tag-color").css("background-color",$("#tag-color").val());$(".btn-save").removeClass("disabled");});function submitTag(){let url="mod_tag";$.post(url,JSON.stringify(TAGS)).done(function(){reloadSettings();$(".btn-save").addClass("disabled");$("#btn-del").addClass("disabled");raiseAlert("Settings are saved","success");return true;}).fail(function(){raiseAlert("Settings are not saved. Please try later","danger");return false;});}
function checkTag(){$(".cat-alert").empty();if(document.forms["add-tag"]["tag_name"].value===""||document.forms["add-tag"]["tag_rule"].value===""||document.forms["add-tag"]["tag_color"].value===""||document.forms["add-tag"]["tag_order"].value===""){$(".cat-alert").html("Fill all the fields in the form!");return false;}
