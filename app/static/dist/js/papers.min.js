...
function renderPapersBase(content,pId){let paper=document.createElement("div");paper.setAttribute("class","paper");paper.setAttribute("id","paper-"+pId);document.getElementById("paper-list-content").appendChild(paper);let ocoinsSpan=document.createElement("span");ocoinsSpan.setAttribute("class","Z3988");ocoinsSpan.setAttribute("title",renderOcoins(content));paper.appendChild(ocoinsSpan);let title=document.createElement("div");title.setAttribute("class","paper-title");title.setAttribute("id","paper-title-"+pId);paper.appendChild(title);let number=document.createElement("span");number.setAttribute("id","paper-num-"+pId);number.textContent=String(pId+1);let titleText=document.createElement("span");titleText.textContent=". "+content.title;title.appendChild(number);title.appendChild(titleText);var tagPanel=document.createElement("div");tagPanel.setAttribute("class","tag-panel");tagPanel.setAttribute("id","tag-panel-"+pId);paper.appendChild(tagPanel);if(content.tags!==undefined){content.tags.forEach(function(tag){var tagDiv=document.createElement("div");tagDiv.setAttribute("class","tag-panel-item");tagDiv.setAttribute("style","background-color:"+TAGS[parseInt(tag,10)].color);tagDiv.textContent=TAGS[parseInt(tag,10)].name;tagPanel.appendChild(tagDiv);});if(content.tags.length===0){tagPanel.setAttribute("style","display: none");}}
let au=document.createElement("div");au.setAttribute("class","paper-au");let authors=content.author;if(authors.length>4){authors=authors.slice(0,5);authors.push('et al');}
au.textContent=authors.join(", ");paper.appendChild(au);let date=document.createElement("div");date.setAttribute("id","paper-date-"+pId);date.setAttribute("class","paper-date");date.textContent=content.date_up;if(content.date_sub!==content.date_up){date.textContent+=" (v1: "+content.date_sub+")";}
